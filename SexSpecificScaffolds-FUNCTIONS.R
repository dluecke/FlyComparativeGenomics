...
#   n_masked: nmasked.csv file with per-scaffold lengths and masked bp counts
#   depth_f: depth_per_scaffold.tsv from female reads against hardmasked assembly
#   depth_m: depth_per_scaffold.tsv from male reads against hardmasked assembly
...
#   varnt_f: nseg_per_scaffold.csv from female reads, count of variant sites of hardmasked scaffolds
#   varnt_m: nseg_per_scaffold.csv from male reads, count of variant sites of hardmasked scaffolds
...
make_df.sexbias_NoReps <- function(l.IN_FILES){
  ...
  NMASKED <- read.csv(l.IN_FILES$n_masked, header = F,
                      col.names = c('scaffold', 'length', 'masked'))
  NMASKED$unmasked <- NMASKED$length - NMASKED$masked
  NMASKED <- NMASKED[order(-NMASKED$unmasked),]
  # input dataframes
  IN_DEPTHF <- read.table(l.IN_FILES$depth_f, header = F, row.names = 1, 
...
ClusterStats <- function(SCAF_LIST, DF.SEXBIAS){
  cbind( nscafs = length(SCAF_LIST), 
         ...
         DF.SEXBIAS[SCAF_LIST,] %>% 
           summarise(total.bp = sum(length, na.rm = T), 
                     unmasked.bp = sum(unmasked, na.rm = T),
                     pr_length = sum(pr_length, na.rm = T),
                     pr_unmasked = sum(pr_unmasked, na.rm = T),
                     # length-weighted average across scaffolds
                     depth.F = weighted.mean(mean_depth.female, unmasked, na.rm = T),
                     depth.M = weighted.mean(mean_depth.male, unmasked, na.rm = T),
                     # frequency across scaffolds
                     var_count.F = sum(mean_nVar.female, na.rm = T), 
                     var_count.M = sum(mean_nVar.male, na.rm = T),
                     var_fr.F = sum(mean_nVar.female, na.rm = T)/sum(unmasked, na.rm = T),
                     var_fr.M = sum(mean_nVar.male, na.rm = T)/sum(unmasked, na.rm = T)
...
make_df.HomologSexBias <- function(CLUSTERS, DF.SEXBIAS.QRY, DF.SEXBIAS.REF){
  
  DF.HOMOLOGS.F <- sapply(CLUSTERS, function(x){ 
      ClusterStats(x$QryScafs, DF.SEXBIAS.QRY) 
    }) %>% t() %>% data.frame()
  
  DF.HOMOLOGS.M <- sapply(CLUSTERS, function(x){
      ClusterStats(x$RefScafs, DF.SEXBIAS.REF)
    }) %>% t() %>% data.frame()
...
make_df.DepthWindows <- function(INFILE_DEPTH, INFILE_NMASKED,
  ...
  DF.DEPTH <- data.frame(
    ...
    position = rownames(DF.NMASKED) %>% str_split_i(pattern = ":", 2) %>% as.integer(),
