...
sort $IN_ASM.fai -nr -k2,2 | \
    cut -f1 | \
    head -n $N_CHR \
    > ${IN_ASM%.*}.scaffolds-autosomes.list

seqtk subseq $IN_ASM ${IN_ASM%.*}.scaffolds-autosomes.list > ${IN_ASM%.*}.autosomes.fa

sort $IN_ASM.fai -nr -k2,2 | \
    cut -f1 | \
    tail -n +$((N_CHR+1)) \
    > ${IN_ASM%.*}.scaffolds-rest.list

seqtk subseq $IN_ASM ${IN_ASM%.*}.scaffolds-rest.list > ${IN_ASM%.*}.rest.fa

ragtag.py splitasm -o ${IN_ASM%.*}.rest.split.agp  ${IN_ASM%.*}.rest.fa > ${IN_ASM%.*}.rest.split.fa
